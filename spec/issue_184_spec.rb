...
describe 'issue 184', unit: true do
  ...
  let(:command_id)  { 123 }
  let(:test_data_xml_template) { ERB.new(File.read("spec/stubs/responses/get_command_output_response.xml.erb")) }
  let(:service) { WinRM::WinRMWebService.new("http://dummy/wsman", :plaintext, :user => "dummy", :pass => "dummy") }
  
  describe 'response doc stdout with invalid UTF-8 characters' do
    let(:test_data_xml) do
      test_data_stdout = 'fff' # Base64-decodes to '}\xF7', an invalid sequence
      test_data_stderr = ''
      test_data_xml_template.result(binding)
    end
  
    before do
      allow(service).to receive(:send_get_output_message).and_return(REXML::Document.new(test_data_xml))
    end
