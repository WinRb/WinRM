...
describe WinRM::Connection do
  context 'new' do
    it 'creates a new winrm session' do
      connection = WinRM::Connection.new(default_session_opts)
  ...
  context 'shell(:not_a_real_shell_type)' do
    it 'raises a descriptive error' do
      connection = WinRM::Connection.new(default_session_opts)
      expect { connection.shell(:not_a_real_shell_type) }.to raise_error(
        'not_a_real_shell_type is not a valid WinRM shell type. ' \
