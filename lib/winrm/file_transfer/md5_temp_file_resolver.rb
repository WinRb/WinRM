...
module WinRM
  class Md5TempFileResolver
    ...
    def temp_file_path(local_file_path, dest_file_path)
      script = temp_file_script(local_file_path, dest_file_path)
      @command_executor.run_powershell(script).to_s.chomp
    end

    private

    def temp_file_script(local_file_path, dest_file_path)
