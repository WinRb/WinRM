...
module WinRM
  ...
  class Service
    ...
    attr_reader :endpoint, :timeout, :transport

    def initialize(endpoint, opts = {})
      default_opts = { transport: :http, timeout: 'PT60S', max_env_sz: 153600, locale: 'en-US', domain: nil }
      opts = default_opts.merge(opts)

      @endpoint = endpoint
      @timeout = opts[:timeout]
      @max_env_sz = opts[:max_env_sz] 
      @locale = opts[:locale]
      
      uri = URI("")
      uri.host = endpoint
      uri.path = "/wsman"
      uri.port = opts[:port] unless opts[:port].nil?

      case opts[:transport].to_sym
      when :kerberos
        require 'gssapi'
        # TODO: check fo keys and throw error if missing
        uri.port = 5985 if opts[:port].nil?
        uri.scheme = 'http'
        @transport = Transport::HttpGSSAPI.new(uri.to_s, opts)
      when :http
        uri.scheme = 'http'
        uri.port = 5985 if opts[:port].nil?
        @transport = Transport::HttpPlaintext.new(uri.to_s, opts)
      when :ssl
        uri.scheme = 'https'
        uri.port = 5986 if opts[:port].nil?
        @transport = Transport::HttpSSL.new(uri.to_s, opts)
      else 
        raise StandardError, "Transport does not exist #{opts[:protocol]}"
      end
    ...
    def get_command_output(shell_id, command_id, &block)
      ...
      (resp/"//#{NS_WIN_SHELL}:Stream").each do |n|
        ...
        if block_given?
          yield stream[:stdout], stream[:stderr]
        else
          output[:data] << stream
        end
      end
      ...
      output.delete(:data) if output[:data].empty?
      output
    end
    ...
    def send_message(message)
      resp = @transport.send_request(message)
